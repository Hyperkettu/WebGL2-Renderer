...
export const prefixVS = 'pbrMorphedTextureTransformVS';
export const prefixFS = 'pbrMorphedTextureTransformFS';
...
const fsSrc =
        ...
        uniform sampler2D targetAlbedo;
        ${ hasNormalMap ? 'uniform sampler2D normalMap;' : ''}
        ${ hasNormalMap ? 'uniform sampler2D targetNormal;' : ''}
		...
		void main() {
            ...
            vec4 albedoWithAlpha2 = texture(targetAlbedo, uvs.st).rgba;
            albedo = mix(albedo, albedoWithAlpha2.rgb, weight);
            ...
            'N = normalize(TBN * N);' +
            'vec3 N2 = (2.0 * texture(targetNormal, uvs.st) - 1.0).rgb;' +
            'N2 = normalize(TBN * N2);' + 
            'N = normalize(mix(N, N2, weight));' :
			...
			outColor.a = mix(albedoWithAlpha.a, albedoWithAlpha2.a, weight);
